...
def normalizeSQLQuery(query, baseDict):
    ...
    for token in parsed.tokens:
        if token.is_whitespace:
            ...
            formatted_query.append(Tp.process_from(il, alias_map))
            Tp.process_from(il, alias_map)
        elif isinstance(token, IdentifierList) and formatted_query and formatted_query[-1] == 'FROM':
            formatted_query.append(Tp.process_from(token, alias_map))
            Tp.process_from(token, alias_map)
    ...
    for token in parsed.tokens:
        if token.is_whitespace:
            ...
            formatted_query.append(Tp.process_groupby(token, alias_map))
            ...
            formatted_query.append(Tp.process_select(token, alias_map))
        else:
